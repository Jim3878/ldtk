...
- 新增外部呼叫入口：可以用命令列參數啟動 LDtk 並直接開啟指定專案、跳到指定 Level、選取指定 Entity（`LDtk.exe <專案路徑> --goto-level=<levelIid> --goto-entity=<entityIid>`，用穩定的 `iid` 定位，不受重新命名或搬移影響）。若 LDtk 已經開著，重新用命令列呼叫不會開新視窗，而是把既有視窗帶到最前面並執行跳轉。
- 修正：額外參數的 `Float`/`Int` 子欄位，只要輸入的數值剛好等於該子欄位設定的預設值，就會被誤判成「沒有設定」而被移除，跟一般欄位「沒設定值」與「值剛好等於預設值」是兩種不同狀態的行為不一致（例如日後把子欄位的預設值改掉，原本刻意填入舊預設值的資料會被靜默套用成新的預設值）。現在跟一般欄位一致：只有清空輸入框才會被視為使用預設值。

# 1.5.8

- 全域搜尋現在能個別搜到 entity 陣列欄位裡的每一個值（先前陣列裡只有第一個值會被搜尋到）。
- 修正：全域搜尋搜不到中文關鍵字。
- 關閉 Grid 模式後，entity 可以自由移動了（先前只有新增 entity 時才會正確套用這個設定，拖曳移動已放置的 entity 仍會被強制貼齊格線）。
- Entity 陣列欄位的「額外參數」（附加在 String 陣列欄位上的隱藏 Bool/Enum/String/Float 子欄位）現在預設一律收合，不再因為已經有值就自動展開；手動展開的項目在你繼續編輯時會保持展開。
- 每個有額外參數的陣列欄位旁新增「Toggle all extra parameters」按鈕，可以一次展開/收合該欄位所有項目的額外參數。
- 額外參數如果還是預設值，現在會有視覺標示（條紋背景、變灰的參數名稱），跟一般欄位一致，並附上對應的「重置為預設值」按鈕。
- 額外參數新增 `Int`（整數）子欄位類型，跟原本的 Bool/Enum/String/Float 並列。
- 修正：復原/重做（Ctrl+Z / Ctrl+Y）有時會強制關閉 Entity 側邊欄，即使被復原/重做的內容跟目前正在編輯的 entity 無關。
- Entity 側邊欄的面板寬度現在會記住上次調整的大小。
